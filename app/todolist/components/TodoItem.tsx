        ...
        <li key={todo.id} className="flex items-center justify-between pt-3 pb-3 pl-3 pr-3 transition duration-300 ease-in-out even:bg-gray-100 md:even:bg-transparent md:hover:bg-gray-100">
                ...
                <div className="flex flex-wrap items-center gap-x-3">
                    <span>
                        {(() => {
                            switch (todo.priority) {
                                case 3:
                                    return (
                                        <svg className="block w-1 h-1 align-middle fill-green-500" viewBox="0 0 6 6" aria-hidden="true">
                                            <circle cx="3" cy="3" r="3"></circle>
                                        </svg>
                                    );
                                case 2:
                                    return (
                                        <svg className="block w-1 h-1 align-middle fill-yellow-300" viewBox="0 0 6 6" aria-hidden="true">
                                            <circle cx="3" cy="3" r="3"></circle>
                                        </svg>
                                    );
                                case 1:
                                    return (
                                        <svg className="block w-1 h-1 align-middle fill-red-500" viewBox="0 0 6 6" aria-hidden="true">
                                            <circle cx="3" cy="3" r="3"></circle>
                                        </svg>
                                    );
                                default:
                    ...
                    <div className="flex">
                        <p className={`w-24 text-center text-xs pt-1 pr-3 pb-1 pl-3 rounded-md border border-opacity-20 ${todo.completed !== true ? "bg-red-100 border-red-500" : "bg-green-100 border-green-500"}`}>
                            <span className={`${todo.completed !== true ? "text-red-600" : "text-green-600"} leading-4 font-normal`}>{todo.completed !== true ? "InComplete" : "Complete"}</span>
                        </p>
            ...
            <div className="flex items-center">
                <Popover className={`relative ${customPopoverStyles.popoverWrap}`}>
                    ...
                    <Popover.Panel className={`absolute z-10 ${customPopoverStyles.popoverPanel}`}>
                        <div className="p-2 space-y-2 bg-white rounded-lg shadow-md">
                            <Popover.Button
                                className={customPopoverStyles.commonButton}
                                onClick={() => {
                ...
                <span className={`${customButtonStyles.commonButtonWrap} bg-opacity-5 rounded-md inline-flex`}>
                    ...
                    <button className={customButtonStyles.commonButton} onClick={() => handleIsEditFormVisible(todo.id, true)}>
                        編集
                    </button>
                    <button className={customButtonStyles.commonButton} onClick={() => handleDelete(todo.id)}>
                        削除
                    </button>
                </span>
            </div>
